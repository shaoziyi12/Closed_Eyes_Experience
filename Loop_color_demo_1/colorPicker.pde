...
class ColorPicker {
  ...
  pickerHeight = 255; // <-- Add pickerHeight property
...
void pickColor(int mouseX, int mouseY) { 
    if (mouseX > ColorPickerX && mouseX < ColorPickerX + 255 && mouseY > ColorPickerY && mouseY < ColorPickerY + 255) {
      CrossX = constrain(mouseX, ColorPickerX, ColorPickerX + 255);
      CrossY = constrain(mouseY, ColorPickerY, ColorPickerY + 255);
      activeColor = color(hue(activeColor), saturation(activeColor), brightness(activeColor));
    }
    if (mouseX > ColorPickerX + 258 && mouseX < ColorPickerX + 276 && mouseY > ColorPickerY && mouseY < ColorPickerY + 255) {
      LineY = constrain(mouseY, ColorPickerY, ColorPickerY + 255);
      activeColor = color(hue(activeColor), (CrossX - ColorPickerX) / 255.0, (255 - (CrossY - ColorPickerY)) / 255.0);
    }
  }
